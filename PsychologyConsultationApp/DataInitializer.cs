using System;
using System.Configuration;
using System.Data.SqlClient;

namespace PsychologyConsultationApp
{
    public static class DataInitializer
    {
        private static readonly string connectionString =
            ConfigurationManager.ConnectionStrings["DbConnection"].ConnectionString;

        public static void EnsureDatabase()
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                EnsureUsersTable(conn);
                EnsureAppointmentsTable(conn);
                SeedSpecialists(conn);
            }
        }

        private static void EnsureUsersTable(SqlConnection conn)
        {
            string createUsers = @"
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Users')
BEGIN
    CREATE TABLE dbo.Users
    (
        Id INT IDENTITY(1,1) PRIMARY KEY,
        FullName NVARCHAR(200) NOT NULL,
        Email NVARCHAR(200) NOT NULL,
        PasswordHash NVARCHAR(200) NOT NULL,
        Role NVARCHAR(50) NOT NULL,
        Specialty NVARCHAR(200) NULL
    );
END
ELSE
BEGIN
    IF COL_LENGTH('Users','Specialty') IS NULL
    BEGIN
        ALTER TABLE dbo.Users ADD Specialty NVARCHAR(200) NULL;
    END
END";

            using (SqlCommand cmd = new SqlCommand(createUsers, conn))
            {
                cmd.ExecuteNonQuery();
            }
        }

        private static void EnsureAppointmentsTable(SqlConnection conn)
        {
            string createAppointments = @"
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Appointments')
BEGIN
    CREATE TABLE dbo.Appointments
    (
        Id INT IDENTITY(1,1) PRIMARY KEY,
        PatientId INT NOT NULL,
        SpecialistId INT NOT NULL,
        AppointmentDate DATE NOT NULL,
        AppointmentTime TIME(0) NOT NULL,
        Status NVARCHAR(50) NOT NULL DEFAULT 'Bekliyor',
        ComplaintText NVARCHAR(MAX) NULL,
        SessionNotes NVARCHAR(MAX) NULL,
        CONSTRAINT FK_Appointments_Patient FOREIGN KEY (PatientId) REFERENCES dbo.Users(Id),
        CONSTRAINT FK_Appointments_Specialist FOREIGN KEY (SpecialistId) REFERENCES dbo.Users(Id)
    );
END
ELSE
BEGIN
    IF COL_LENGTH('Appointments','ComplaintText') IS NULL
    BEGIN
        ALTER TABLE dbo.Appointments ADD ComplaintText NVARCHAR(MAX) NULL;
    END
    IF COL_LENGTH('Appointments','SessionNotes') IS NULL
    BEGIN
        ALTER TABLE dbo.Appointments ADD SessionNotes NVARCHAR(MAX) NULL;
    END
END";

            using (SqlCommand cmd = new SqlCommand(createAppointments, conn))
            {
                cmd.ExecuteNonQuery();
            }
        }

        private static void SeedSpecialists(SqlConnection conn)
        {
            var specialists = new (int Id, string Name, string Specialty)[]
            {
                (101, "Dr. Ayşe Demir", "Psikolog - Anksiyete"),
                (102, "Dr. Mehmet Yıldız", "Psikiyatrist - Depresyon"),
                (103, "Dr. Elif Kaya", "Psikolog - Uyku"),
                (104, "Dr. Caner Koç", "Psikiyatrist - Sınav Stresi"),
                (105, "Dr. Selin Acar", "Psikolog - Çocuk & Ergen")
            };

            foreach (var specialist in specialists)
            {
                string upsert = @"
IF NOT EXISTS (SELECT 1 FROM dbo.Users WHERE Id=@id)
BEGIN
    INSERT INTO dbo.Users (Id, FullName, Email, PasswordHash, Role, Specialty)
    VALUES (@id, @name, CONCAT('uzman', @id, '@mail.com'), '1234', 'Uzman', @specialty);
END
ELSE
BEGIN
    UPDATE dbo.Users
    SET FullName=@name, Specialty=@specialty, Role='Uzman'
    WHERE Id=@id;
END";

                using (SqlCommand cmd = new SqlCommand(upsert, conn))
                {
                    cmd.Parameters.AddWithValue("@id", specialist.Id);
                    cmd.Parameters.AddWithValue("@name", specialist.Name);
                    cmd.Parameters.AddWithValue("@specialty", specialist.Specialty);
                    cmd.ExecuteNonQuery();
                }
            }
        }
    }
}
